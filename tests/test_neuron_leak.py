...
def test_neuron_leak_config():
    ...
    snn = nn.Sequential(
        ...
        nn.Conv2d(in_channels=2, out_channels=8, kernel_size=(1, 1), stride=(2, 2),  bias=True),
        IAFSqueeze(min_v_mem=-1.0, spike_threshold=1.0, batch_size=1),

        nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(1, 1), stride=(2, 2), bias=True),
        IAFSqueeze(min_v_mem=-1.0, spike_threshold=1.0, batch_size=1),
    )

    # generate samna config object based on the torch.nn.Module
    dynapcnn = DynapcnnNetwork(snn=snn, discretize=True, dvs_input=True, input_shape=(1, 64, 64))
...
def test_neuron_leak():
    ...
    for device_name, device_info in devices.items():
        dynapcnn = DynapcnnNetwork(snn=snn, discretize=True, dvs_input=False, input_shape=(1, 4, 4))
        ...
        for x in range(4):
            for y in range(4):
                ...
                ev.layer = 0
                ev.address = calculate_neuron_address(x=x, y=y, c=0, feature_map_size=(1, 4, 4))
        ...
        for iter_times in range(5):
            ...
            for out_ev in output_events:
                c, x, y = neuron_address_to_cxy(out_ev.address, feature_map_size=(1, 4, 4))
                pre_neuron_state = neuron_states.get((c, x, y), 127)
                assert pre_neuron_state > out_ev.neuron_state, "Neuron V_Mem doesn't decrease!"
