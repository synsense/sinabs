...
class SCNN(nn.Module):
    def __init__(self, input_size, nb_classes, batch_size, surrogate_fn, min_v_mem=-0.313, spk_thr=2.0) -> None:
        ...
        self.fc4 = nn.Linear(100, 64, bias=False)
    ...
    def forward(self, x):
        ...
        merge3_out = self.merge3(iaf4_out, iaf5_out)

        fc3_out = self.fc3(merge3_out)
        iaf6_out = self.iaf6(fc3_out)

        fc4_out = self.fc4(iaf6_out)
        iaf7_out = self.iaf7(fc4_out)

        merge4_out = self.merge4(iaf6_out, iaf7_out)

        fc5_out = self.fc5(merge4_out)
