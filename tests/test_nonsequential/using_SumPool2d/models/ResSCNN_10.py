...
class SCNN(nn.Module):
    ...
    def rescale_conv_weights(self, rescale_fn, lambda_):
        ...
        rescale_fn(self.conv7, [(4,4)], lambda_)
    ...
    def forward(self, x):
        ...
        pool2a_out = self.pool2a(iaf2_out)    # to CONV 5
        #  -- conv block 3 ---
        ...
        pool4a_out = self.pool4a(iaf4_out)    # to CONV 7
        #  -- conv block 5 ---
        #print(pool2a_out.shape, pool4_out.shape)
        merge2_out = self.merge2(pool2a_out, pool4_out)
        ...
        #  -- conv block 7 ---
        #print(pool4a_out.shape, iaf6_out.shape)
        merge4_out = self.merge4(pool4a_out, iaf6_out)
        conv7_out = self.conv7(merge4_out)
        iaf7_out = self.iaf7(conv7_out)
        pool7_out = self.pool7(iaf7_out)
        #  -- conv block 8 ---
        #print(pool5a_out.shape, pool7_out.shape)
        merge5_out = self.merge5(pool5a_out, pool7_out)
        conv8_out = self.conv8(merge5_out)
