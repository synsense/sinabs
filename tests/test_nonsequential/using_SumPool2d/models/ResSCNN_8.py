...
class SCNN(nn.Module):
    def __init__(self, input_size, nb_classes, batch_size, surrogate_fn, min_v_mem=-0.313, spk_thr=2.0) -> None:
        ...
        self.merge_fc1a = sl.Merge()
        self.merge_fc1b = sl.Merge()
    ...
    def rescale_conv_weights(self, rescale_fn, lambda_):
        ...
        rescale_fn(self.conv4, [(8,8), (2,2)], lambda_)
    ...
    def forward(self, x):
        ...
        pool1b_out = self.pool1b(iaf1_out)
        # conv 2
        conv2_out = self.conv2(pool1_out)
        iaf2_out = self.iaf2(conv2_out)
        pool2_out = self.pool2(iaf2_out)
        # conv 3
        merge_1a_out = self.merge1a(pool1a_out, pool2_out)
        conv3_out = self.conv3(merge_1a_out)
        iaf3_out = self.iaf3(conv3_out)
        pool3_out = self.pool3(iaf3_out)
        # conv 4
        merge_1b_out = self.merge1b(pool1b_out, pool3_out)
        conv4_out = self.conv4(merge_1b_out)
        ...
        # fc 3
        merge_fc1a_out = self.merge_fc1a(iaf1_fc_out, iaf2_fc_out)
        fc3_out = self.fc3(merge_fc1a_out)
        iaf3_fc_out = self.iaf3_fc(fc3_out)
        # fc 4
        merge_fc1b_out = self.merge_fc1b(iaf1_fc_out, iaf3_fc_out)
        fc4_out = self.fc4(merge_fc1b_out)
