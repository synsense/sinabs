...
class SCNN(nn.Module):
    def __init__(self, input_size, nb_classes, batch_size, surrogate_fn, min_v_mem=-0.313, spk_thr=2.0):
        super().__init__()
        ...
        self.merge5 = sl.Merge()
        self.merge6 = sl.Merge()
    ...
    def rescale_conv_weights(self, rescale_fn, lambda_):
        rescale_fn(self.conv5, [(2,2), (2,2)], lambda_)
        rescale_fn(self.conv6, [(4,4), (2,2)], lambda_)
        rescale_fn(self.conv7, [(2,2)], lambda_)
        rescale_fn(self.conv8, [(2,2), (2,2)], lambda_)
    ...
    def forward(self, x):
        ...
        iaf2_out = self.iaf2(conv2_out)         # to CONV 4
        #  -- conv block 3 ---
        #print(iaf1_out.shape, iaf2_out.shape)
        merge1_out = self.merge1(iaf1_out, iaf2_out)
        conv3_out = self.conv3(merge1_out)
        iaf3_out = self.iaf3(conv3_out)
        pool3a_out = self.pool3a(iaf3_out)  # to CONV 5
        #  -- conv block 4 ---
        #print(iaf2_out.shape, iaf3_out.shape)
        merge2_out = self.merge2(iaf2_out, iaf3_out)
        conv4_out = self.conv4(merge2_out)
        iaf4_out = self.iaf4(conv4_out)
        pool4_out = self.pool4(iaf4_out)
        pool4a_out = self.pool4a(iaf4_out)   # to CONV 6
        #  -- conv block 5 ---
        #print(pool3a_out.shape, pool4_out.shape)
        merge3_out = self.merge3(pool3a_out, pool4_out)
        conv5_out = self.conv5(merge3_out)
        iaf5_out = self.iaf5(conv5_out)
        pool5_out = self.pool5(iaf5_out)
        pool5a_out = self.pool5a(iaf5_out)   # to CONV 7
        #  -- conv block 6 ---
        #print(pool4a_out.shape, pool5_out.shape)
        merge4_out = self.merge4(pool4a_out, pool5_out)
        conv6_out = self.conv6(merge4_out)
        iaf6_out = self.iaf6(conv6_out)
        pool6a_out = self.pool6a(iaf6_out)   # to CONV 8
        #  -- conv block 7 ---
        #print(pool5a_out.shape, iaf6_out.shape)
        merge5_out = self.merge5(pool5a_out, iaf6_out)
        conv7_out = self.conv7(merge5_out)
        iaf7_out = self.iaf7(conv7_out)
        pool7_out = self.pool7(iaf7_out)
        #  -- conv block 8 ---
        #print(pool6a_out.shape, pool7_out.shape)
        merge6_out = self.merge6(pool6a_out, pool7_out)
        conv8_out = self.conv8(merge6_out)
