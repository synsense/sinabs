...
class SCNN(nn.Module):
    def __init__(self, input_size, nb_classes, batch_size, surrogate_fn, min_v_mem=-1.0, spk_thr=1.0) -> None:
        ...
        self.conv4 = nn.Conv2d(16, 16, 2, 1, bias=False)
        ...
        self.fc1 = nn.Linear(flat_s, 1024, bias=False)
        self.iaf4 = IAFSqueeze(batch_size=batch_size, min_v_mem=min_v_mem, surrogate_grad_fn=surrogate_fn, spike_threshold=spk_thr)
        ...
        self.fc5 = nn.Linear(64, nb_classes, bias=False)
    ...
    def get_flatten_size(input_size):
        conv1_dims = SCNN.conv2d_output_size(input_size, 1, (2, 2))
        pool1_dims = SCNN.pool_output_size(conv1_dims, 1, (2, 2))

        conv2_dims = SCNN.conv2d_output_size(pool1_dims, 8, (2, 2))
        pool2_dims = SCNN.pool_output_size(conv2_dims, 8, (2, 2))

        conv3_dims = SCNN.conv2d_output_size(pool2_dims, 16, (2, 2))
        pool3_dims = SCNN.pool_output_size(conv3_dims, 16, (2, 2))

        conv4_dims = SCNN.conv2d_output_size(pool3_dims, 16, (2, 2))
        pool4_dims = SCNN.pool_output_size(conv4_dims, 16, (2, 2))
    ...
    def forward(self, x):
        ...
        fc5_out = self.fc5(iaf7_out)
        iaf8_out = self.iaf8(fc5_out)
