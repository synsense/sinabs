...
def test_no_kill_bits():
    ...
    dynapcnndevkit_config = network.make_config(chip_layers_ordering=[0, 1], device="dynapcnndevkit:0")
    # Check that for weight values equal to zero, the kill bit is enabled
    assert np.sum(dynapcnndevkit_config.cnn_layers[0].weights_kill_bit) == 2*10*3*3
