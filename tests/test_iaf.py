...
from sinabs.layers import IAF, IAFSqueeze, IAFRecurrent, IAFRecurrentSqueeze
import pytest
import numpy as np
...
def test_iaf_recurrent():
    ...
    rec_connectivity[1].weight = nn.Parameter(torch.ones(n_neurons,n_neurons)/n_neurons*0.5)
    layer = IAFRecurrent(rec_connectivity=rec_connectivity)
    spike_output = layer(input_current)
...
def test_iaf_recurrent_squeezed():
    batch_size = 10
    time_steps = 100
    input_dimensions = (batch_size*time_steps, 2, 7, 7)
    n_neurons = np.product(input_dimensions[1:])
    input_current = torch.rand(*input_dimensions)

    rec_connectivity = nn.Sequential(nn.Flatten(), nn.Linear(n_neurons, n_neurons, bias=False))
    rec_connectivity[1].weight = nn.Parameter(torch.ones(n_neurons,n_neurons)/n_neurons*0.5)
    layer = IAFRecurrentSqueeze(batch_size=batch_size, rec_connectivity=rec_connectivity)
    spike_output = layer(input_current)
