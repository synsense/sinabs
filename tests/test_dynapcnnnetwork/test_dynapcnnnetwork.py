...
@pytest.mark.parametrize("snn, input_shape, batch_size, expected_output", args_DynapcnnNetworkTest)
def test_DynapcnnNetwork(snn, input_shape, batch_size, expected_output):
    ...
    assert expected_output['dcnnl_edges'] == dcnnnet.dcnnl_edges, \
        f'wrong list of edges describing DynapcnnLayer connectivity.'
    
    for node, args in dcnnnet.merge_points.items():

        assert node in expected_output['merge_points'], \
            f'DynapcnnLayer {node} is not a merge point.'
        assert args['sources'] == expected_output['merge_points'][node]['sources'], \
    ...
    assert expected_output['output_shape'] == output.shape, \
