...
SPIKE_LAYERS = [name for name, child in CNN.named_children() if isinstance(child, (nn.ReLU,))]
PARAM_LAYERS = [name for name, child in CNN.named_children() if isinstance(child, (nn.Conv2d, nn.Linear))]


def test_normalize_weights_cpu():
    # test on cpu tensor
    sample_input_cpu = torch.rand(8, 1, 28, 28)
    normalize_weights(CNN.cpu(), sample_input_cpu, SPIKE_LAYERS, SPIKE_LAYERS, percentile=100)
