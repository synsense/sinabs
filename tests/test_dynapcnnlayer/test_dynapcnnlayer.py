...
import pytest
from sinabs.backend.dynapcnn.utils import construct_dynapcnnlayer, construct_layerhandler, update_nodes_io
...
def test_DynapcnnLayer(nodes_to_dcnnl_map, dpcnnl_idx, sinabs_edges, entry_point, expected_output):
    ...
    node, output_shape = layerhandler.get_modified_node_io(nodes_to_dcnnl_map[dpcnnl_idx])
    ...
    assert layerhandler.conv_node_id == expected_output[dpcnnl_idx]['conv_node_id'], \
        f'wrong \'DynapcnnLayer.conv_node_id\': convolution layer should be node {conv_node_id}.'
    assert layerhandler.conv_in_shape == expected_output[dpcnnl_idx]['conv_in_shape'], \
    ...
    assert layerhandler.spk_node_id == expected_output[dpcnnl_idx]['spk_node_id'], \
        f'wrong \'DynapcnnLayer.spk_node_id\': spiking layer should be node {spk_node_id}.'
    assert layerhandler.pool_node_id == expected_output[dpcnnl_idx]['pool_node_id'], \
        f'wrong \'DynapcnnLayer.pool_node_id\': pooling layer node(s) should be {pool_node_id}.'
    assert layerhandler.conv_rescaling_factor == expected_output[dpcnnl_idx]['conv_rescaling_factor'], \
        f'wrong \'DynapcnnLayer.conv_rescaling_factor\': computed re-scaling factor should be {conv_rescaling_factor}.'
    assert layerhandler.dynapcnnlayer_destination == expected_output[dpcnnl_idx]['dynapcnnlayer_destination'], \
    ...
    assert layerhandler.nodes_destinations == expected_output[dpcnnl_idx]['nodes_destinations'], \
    ...
    assert layerhandler.entry_point == expected_output[dpcnnl_idx]['entry_point'], \
