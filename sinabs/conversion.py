...
def replace_module(model: nn.Module, source_class: type, mapper_fn: Callable):
    ...
    replace_module_inner(new_model, source_class, mapper_fn)
    return new_model


def replace_module_inner(model: nn.Sequential, source_class: type, mapper_fn: Callable):
    for name, module in model.named_children():
        if list(module.named_children()):
            replace_module_inner(module, source_class, mapper_fn)
