...
class ActivationFunction:
    ...
    function. The default is an activation function that uses multiple spikes
    ...
    def __call__(
        ...
        spikes = self.spike_fn.apply(
            *input_tensors, spike_threshold, self.surrogate_grad_fn
        )
        state = self.reset_fn(spikes, state, spike_threshold)
        return spikes, state
